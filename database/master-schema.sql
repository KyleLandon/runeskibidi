...
-- Ensure required extensions are available (Supabase usually has these)
CREATE EXTENSION IF NOT EXISTS pgcrypto;

-- Harden default privileges for sequences created in this schema
...
-- Enforce unique email in the mirror table to match auth uniqueness
DO $$ BEGIN
  ALTER TABLE players ADD CONSTRAINT players_email_unique UNIQUE (email);
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
-- Basic username format guard (3-20, letters/numbers/underscore)
DO $$ BEGIN
  ALTER TABLE players
    ADD CONSTRAINT username_format_chk CHECK (username ~ '^[A-Za-z0-9_]{3,20}$');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;
...
-- Unique per player character name
DO $$ BEGIN
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_character_skills_unique ON character_skills(character_id, skill_name);
...
CREATE INDEX IF NOT EXISTS idx_chat_messages_channel_time ON chat_messages(channel, timestamp DESC);
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_guilds_name ON guilds(name);
...
ALTER TABLE world_objects REPLICA IDENTITY FULL;
...
CREATE UNIQUE INDEX IF NOT EXISTS ux_bank_accounts_player ON bank_accounts(player_id);
...
-- Auctions policies: public read, owners manage their auctions
DROP POLICY IF EXISTS "Anyone can view auctions" ON auctions;
DROP POLICY IF EXISTS "Users manage own auctions" ON auctions;
CREATE POLICY "Anyone can view auctions" ON auctions FOR SELECT USING (true);
CREATE POLICY "Users manage own auctions" ON auctions
  USING (seller_id = auth.uid()) WITH CHECK (seller_id = auth.uid());

-- Bank accounts: only owners read/update
DROP POLICY IF EXISTS "Users manage own bank accounts" ON bank_accounts;
CREATE POLICY "Users manage own bank accounts" ON bank_accounts
...
CREATE POLICY "Anyone can view world objects" ON world_objects
    FOR SELECT USING (true);
-- No public insert/update/delete policies for world_objects (service role only)
...
-- Guilds and members: public read, members manage membership records, owners update guild
...
DROP POLICY IF EXISTS "Members can view guild membership" ON guild_members;
DROP POLICY IF EXISTS "Members manage own membership" ON guild_members;
CREATE POLICY "Members can view guild membership" ON guild_members
  FOR SELECT USING (
    player_id = auth.uid() OR guild_id IN (
      SELECT guild_id FROM guild_members WHERE player_id = auth.uid()
    )
  );
CREATE POLICY "Members manage own membership" ON guild_members
  USING (player_id = auth.uid()) WITH CHECK (player_id = auth.uid());
